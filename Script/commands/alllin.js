...
module.exports.run = async function({ api, event, args, Threads }) {
	...
	if (args[0] == "صورة") {
		if (event.type != "message_reply") return api.sendMessage("الرجاء الرد على صورة واحدة",event.threadID,event.messageID);
  	...
  	if (args[0] == "ادمن") {
		try {
			...
			if (!mention) {
				...
				(!find_) ? api.sendMessage("أنت لست مسؤولاً",event.threadID,event.messageID) : (!find) ? api.changeAdminStatus(event.threadID,data,true) : api.changeAdminStatus(event.threadID,data,false);
			}
			else {
				const threadAdmins = await Threads.getInfo(event.threadID);
      	const find_ = threadAdmins.adminIDs.find(el => el.id == event.senderID);
				const find = threadAdmins.adminIDs.find(el => el.id == mention);
				(!find_) ? api.sendMessage("أنت لست مسؤولاً",event.threadID,event.messageID) : (!find) ? api.changeAdminStatus(event.threadID,mention,true) : api.changeAdminStatus(event.threadID,mention,false);
			}
		}
		catch (e) {
			return api.sendMessage("هناك أخطاء تحدث!",event.threadID,event.messageID);
		}
 	}
}
